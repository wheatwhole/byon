...
- **Inline writing and page editing:** On accounts where Notion renders its native **Space to edit with AI** writer, BYON borrows that live surface after submission and prevents the native Notion AI request. Standalone writing stays inside the borrowed writer until **Insert below** or **Accept** fills the empty line where the writer opened. The finished-state controls sit in their own right-aligned row below the preview, matching Notion's native generated-content hierarchy. BYON submits the complete generated document through one native paste transaction, allowing Notion to create headings, paragraphs, lists, tasks, quotes, code, tables, images, and dividers together without losing the active editor between commands. It verifies that the complete result remains present before accepting the transaction, and never follows a partially applied paste with an overlapping fallback insertion. Only when Notion declines the paste without changing the page does BYON insert readable plain text through the editor. Draft and insertion previews use the same Markdown renderer, while replacements use token-level diffs inside the borrowed native writer. The live Notion page remains untouched until **Accept**, avoiding conflicts with Notion's React mutation recovery.
...
BYON now gives the provider each selected tool's original live MCP JSON Schema, including unions, references, constraints, and command-specific requirements. It also forwards the MCP server's initialization instructions. Only when an OpenAI-compatible endpoint explicitly rejects that schema does BYON retry with a conservative normalized schema, followed by a JSON-string envelope as the final compatibility fallback; both fallbacks retain the original schema in the tool description. Before sending a call to Notion, BYON validates decoded arguments against the original live schema. Malformed JSON, missing required fields, unsupported fields, wrong types, invalid constrained values, and server-side tool errors are returned as structured correction feedback rather than being treated as successful work.

Notion tool turns include the user's current local date, time, and timezone so relative requests such as “today” are resolved consistently. Tool guidance favors targeted search followed by fetch, follows child references for nested content, and prefers data-source or database-view queries for database items when those tools are available. For Notion write tools, BYON reads the server-advertised enhanced-Markdown resource when available. Page edits fetch before writing and prefer targeted `update_content` operations. BYON never recommends bypassing Notion's child-page/database deletion guard; repeated failed calls are compared without volatile request IDs so identical failures stop cleanly. An unsubmitted answer draft is corrected inside the existing tool loop without re-running tool selection; a new routing turn is reserved for verifier feedback that may require a different tool set.
